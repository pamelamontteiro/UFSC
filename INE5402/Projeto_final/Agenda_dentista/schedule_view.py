...
class ScheduleView:
    ...
    def create_screen(self):
        ...
        self.canvas.create_image(320.0, 470.0, image=background)
    ...
    def create_inputs(self):
        ...
        self.entry_dentist = ttk.OptionMenu(
            self.canvas, self.dentist_var, *self.dentist_names, style="BW.TMenubutton"
        )
        ...
        self.canvas.create_image(700.0, 250.0, image=input_appointment_day)
        ...
        self.entry_appointment_day.place(x=576.0, y=233.0, width=246.0, height=34.0)
        ...
        self.canvas.create_rectangle(
            572.0, 283.0, 828.0, 390.0 + 5 * 27, fill=COLORS["WHITE"]
        )
            ...
            if dentist:
                for idx, schedule in enumerate(dentist.get_schedule(day)):
                    self.dentist_schedule.insert(
                        idx + 1,
                        (
                            f"{schedule[0]}:00 - {schedule[1]}:00 - {schedule[2].get_name()}"
                            if len(schedule) == 3
                            else schedule
                        ),
                    )
