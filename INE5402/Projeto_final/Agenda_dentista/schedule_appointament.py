...
class ScheduleAppointament:
    ...
    def __init__(self, root, dentists, clients):
        ...
        def on_cpf_change(event):
            ...
            client = next(
                (client for client in self.clients if client.get_cpf() == cpf), None
            )
            if client:
        ...
        def on_day_change(event):
            ...
            dentist = next(
                (
                    ...
                    if dentist.get_name() == dentist_name
                    and dentist.get_cro() == dentist_cro
                ),
                None,
            )
            ...
            if dentist:
                for idx, schedule in enumerate(dentist.get_schedule(day)):
                    self.dentist_schedule.insert(
                        idx + 1,
                        (
                            f"{schedule[0]}:00 - {schedule[1]}:00"
                            if len(schedule) == 3
                            else schedule
                        ),
                    )
    ...
    def add_appointament(self, dentist_name, day, start_hour, end_hour, client_cpf):
        ...
        dentist = [
            dentist
            for dentist in self.dentists
            if dentist.get_name() == dentist_name and dentist.get_cro() == dentist_cro
        ]
        dentist_index = next(
            (
                i
                for i, dentist in enumerate(self.dentists)
                if dentist.get_name() == dentist_name
                and dentist.get_cro() == dentist_cro
            ),
            None,
        ...
        if dentist and client:
            ...
            if dentist.add_schedule(day, start_hour, end_hour, client):
                ...
                messagebox.showerror(
                    "Erro",
                    f"Horário indisponível! O horário do dentista vai das {dentist.get_working_hour()[0]}:00 às {dentist.get_working_hour()[1]}:00!",
                )
    ...
    def create_inputs(self):
        ...
        self.canvas.create_image(700.0, 200.0, image=input_dentist)
        style = ttk.Style()
        style.configure(
            "BW.TMenubutton", foreground=COLORS["WHITE"], background=COLORS["BLUE_BG"]
        )
        ...
        self.entry_dentist.place(x=576.0, y=183.0, width=246.0, height=34.0)
        ...
        self.canvas.create_image(700.0, 300.0, image=input_client_name)
        ...
        self.entry_client_name.place(x=576.0, y=283.0, width=246.0, height=34.0)
        ...
        self.canvas.create_image(700.0, 350.0, image=input_appointment_day)
        ...
        self.entry_appointment_day.place(x=576.0, y=333.0, width=246.0, height=34.0)
        ...
        self.canvas.create_rectangle(
            572.0, 383.0, 828.0, 390.0 + 5 * 14.0, fill=COLORS["WHITE"]
        )
        ...
        self.canvas.create_image(700.0, 500.0, image=input_start_hour)
        ...
        self.entry_start_hour.place(x=576.0, y=483.0, width=246.0, height=34.0)
        ...
        self.canvas.create_image(700.0, 550.0, image=input_end_hour)
        ...
        self.entry_end_hour.place(x=576.0, y=533.0, width=246.0, height=34.0)
    ...
    def create_buttons(self):
        ...
        add_btn = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.add_appointament(
                self.client_dentist.get(),
                self.appointament_day.get(),
                self.start_hour.get(),
                self.end_hour.get(),
                self.client_cpf.get(),
            ),
        ...
        add_btn.place(x=650.0, y=583.0, width=180.0, height=56.0)
